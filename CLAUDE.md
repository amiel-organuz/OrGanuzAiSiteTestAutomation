...
The default `npx playwright test` runs 58 tests: 45 green (chromium 12, product 31, organuz-api 1, agent 1) plus 13 live per-role specs (product-setup 3 + product-authenticated 10) that **skip** without per-role credentials. (Of the 31 product tests, token-sanity 3 + public-app-sanity 5 open the live dev app and skip only on a genuine dev outage.) GitHub Actions runs the identical set — keep the two in sync per the **`test-suite-parity`** skill.
