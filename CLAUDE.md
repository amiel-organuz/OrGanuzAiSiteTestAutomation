...
- **`monitoring`** (opt-in) → `tests/monitoring/**` → dedicated availability/contract monitoring for the critical external map dependencies **Govmap** (`www.govmap.gov.il` — map API + address geocoding) and **Ofek** (`basemaps.govmap.gov.il` — Survey-of-Israel orthophoto tiles), 25 checks each. Uses `APIRequestContext` (no browser). **Not in the default suite**: registered only when `MONITORING_ENABLED=true` (`npm run test:monitoring`), and it is *meant to fail* when a dependency breaks — that is the alert. Runs on a 30-min cron in `.github/workflows/monitoring.yml`, separate from the PR gate; a failure alerts via an auto-managed GitHub issue + Slack (both webhooks), and the next green run auto-closes the issue. Endpoints/tokens/tile coords live in `config.json → monitoring`.
